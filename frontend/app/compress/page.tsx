        ...
        <div className="flex flex-col items-center pb-32 pt-16 px-4">
            ...
            {!file && status === "idle" ? (
                <div className={`w-full max-w-4xl h-[300px] border-4 border-dashed rounded-3xl flex flex-col items-center justify-center transition-all cursor-pointer ${isHovering ? "border-pdfred bg-red-50 dark:bg-red-900/20 shadow-inner" : "border-pdfred/40 bg-white dark:bg-[#1e1e1e] hover:bg-gray-50 dark:hover:bg-gray-800 hover:border-pdfred hover:shadow-lg"}`} onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop} onClick={() => fileInputRef.current?.click()}>
                    ...
                    <button className="bg-pdfred text-white font-bold py-4 px-10 rounded-2xl text-xl shadow-lg hover:scale-105 transition-transform">Selecionar PDF</button>
                ...
                <div className="w-full max-w-md bg-white dark:bg-[#1e1e1e] rounded-3xl shadow-xl border border-gray-100 dark:border-gray-800 p-8 flex flex-col items-center text-center">
                    {status === "idle" && (
                        <>
                            <FileArchive className="text-pdfred w-16 h-16 mb-4" />
                            <p className="text-xl font-bold text-gray-800 dark:text-gray-200 truncate w-full">{file!.name}</p>
                            <div className="mt-8 flex flex-col gap-4 w-full">
                                <button onClick={handleCompress} className="bg-pdfred text-white font-bold py-4 px-8 rounded-xl hover:bg-red-700 w-full shadow-md transition-colors">Comprimir Arquivo</button>
            ...
            {status === "success" && (
                <div className="flex flex-col items-center animate-in zoom-in bg-white dark:bg-[#1e1e1e] p-12 rounded-3xl shadow-2xl border border-green-50 dark:border-green-900/30 w-full max-w-2xl mt-10">
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 w-full px-4 mt-6">
                        ...
                        <button onClick={() => { setFile(null); setStatus("idle"); setDownloadUrl(null); }} className="flex-1 bg-pdfred hover:bg-red-700 text-white font-bold py-5 px-6 rounded-2xl flex items-center justify-center shadow-xl transition-all hover:-translate-y-1">
                            Comprimir Outro
                        </button>
                    </div>
                </div>
            ...
            {status === "error" && (
                <div className="flex flex-col items-center bg-white dark:bg-[#1e1e1e] border border-gray-100 dark:border-gray-800 p-10 rounded-3xl shadow-xl mt-10">
                    <div className="text-red-500 font-bold mb-4 text-xl">Erro na compressão.</div>
                    <button onClick={() => setStatus("idle")} className="mt-4 border-2 border-red-500 text-red-500 font-bold py-3 px-8 rounded-xl hover:bg-red-50 dark:hover:bg-red-900/30 transition-colors">Tentar Novamente</button>
                </div>
            )}
        </div>
    );
