...
export default function ConvertToExcel() {
    ...
    return (
        <div className="flex flex-col items-center pb-32 pt-16 px-4">
            ...
            {!file && status === "idle" ? (
                <div className={`w-full max-w-4xl h-[300px] border-4 border-dashed rounded-3xl flex flex-col items-center justify-center transition-all cursor-pointer ${isHovering ? "border-[#2980f2]/50 bg-[#2980f2]/5 dark:bg-[#2980f2]/10 shadow-inner" : "border-[#2980f2]/50/40 bg-white dark:bg-[#1e1e1e] hover:bg-gray-50 dark:hover:bg-gray-800 hover:border-[#2980f2]/50 hover:shadow-lg"}`} onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop} onClick={() => fileInputRef.current?.click()}>
                ...
                <div className="w-full max-w-md bg-white dark:bg-[#1e1e1e] rounded-3xl shadow-xl border border-gray-100 dark:border-gray-800 p-8 flex flex-col items-center text-center transition-all">
                    {status === "idle" && (
                        <>
                            <FileSpreadsheet className="text-[#2980f2] w-16 h-16 mb-4" />
                            <p className="text-xl font-bold text-gray-800 dark:text-gray-200 truncate max-w-full">{file!.name}</p>
                            <div className="mt-8 flex flex-col gap-4 w-full">
                                <button onClick={handleConvert} className="bg-[#2980f2] text-white font-bold py-4 px-8 rounded-xl flex items-center justify-center gap-2 hover:bg-[#2980f2]/90 w-full shadow-md"><Table /> Extrair para EXCEL</button>
                    ...
                    {(status === "uploading" || status === "processing") && (
                        <div className="flex flex-col items-center w-full">
                            ...
                            <div className="w-full h-3 bg-gray-100 dark:bg-gray-800 rounded-full mt-4 overflow-hidden relative">
                                <div 
                                    className="h-full bg-[#2980f2] rounded-full transition-all duration-1000 ease-out"
                            ...
                            <button onClick={handleCancelTransfer} className="mt-8 border-2 border-[#2980f2]/50 text-[#2980f2] font-bold py-3 px-8 rounded-xl hover:bg-[#2980f2]/5 dark:hover:bg-[#2980f2]/10 transition w-full">
                                Cancelar Extração
                            </button>
                        </div>
                    )}
            ...
            {status === "error" && (
                <div className="flex flex-col items-center bg-white dark:bg-[#1e1e1e] p-10 rounded-3xl shadow-xl border border-gray-100 dark:border-gray-800 mt-10">
                    <div className="text-[#2980f2] font-bold mb-4 text-xl">Falha na extração ou PDF sem tabelas estruturadas.</div>
                    <button onClick={handleReset} className="mt-4 border-2 border-[#2980f2]/50 text-[#2980f2] font-bold py-3 px-8 rounded-xl hover:bg-[#2980f2]/5 dark:hover:bg-[#2980f2]/10 transition-colors">Tentar Novamente</button>
                </div>
            )}
        </div>
    );
