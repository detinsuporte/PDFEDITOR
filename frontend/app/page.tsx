...
import { Type, Split, FileArchive, Settings, RefreshCw, FileText, ScanText, PenTool, Lock, Unlock, Stamp } from 'lucide-react';
...
export default function Dashboard() {
    const tools = [
        ...
        { name: "Marca D'Água Global", desc: "Aplica um texto diagonal translucido em todas as páginas do PDF simultaneamente.", icon: <Stamp className="w-10 h-10 text-orange-600" />, href: "/watermark", color: "orange" },
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-6xl w-full px-4">
                {tools.map((tool, idx) => (
                    <Link href={tool.href} key={idx} className="bg-white dark:bg-[#1e1e1e] p-8 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 hover:shadow-xl dark:hover:border-gray-700 transition-all group flex flex-col items-start hover:-translate-y-1">
                        <div className={`mb-6 ${tool.color === 'purple' ? 'bg-purple-50 dark:bg-purple-900/20 group-hover:bg-purple-100 dark:group-hover:bg-purple-900/40' : tool.color === 'blue' ? 'bg-blue-50 dark:bg-blue-900/20 group-hover:bg-blue-100 dark:group-hover:bg-blue-900/40' : tool.color === 'green' ? 'bg-green-50 dark:bg-green-900/20 group-hover:bg-green-100 dark:group-hover:bg-green-900/40' : tool.color === 'gray' ? 'bg-gray-100 dark:bg-gray-800 group-hover:bg-gray-200 dark:group-hover:bg-gray-700' : tool.color === 'orange' ? 'bg-orange-50 dark:bg-orange-900/20 group-hover:bg-orange-100 dark:group-hover:bg-orange-900/40' : 'bg-red-50 dark:bg-red-900/20 group-hover:bg-red-100 dark:group-hover:bg-red-900/40'} p-4 rounded-xl transition-colors`}>{tool.icon}</div>
                        <h3 className={`text-2xl font-bold text-gray-800 dark:text-gray-100 mb-3 ${tool.color === 'purple' ? 'group-hover:text-purple-600 dark:group-hover:text-purple-400' : tool.color === 'blue' ? 'group-hover:text-blue-600 dark:group-hover:text-blue-400' : tool.color === 'green' ? 'group-hover:text-green-600 dark:group-hover:text-green-400' : tool.color === 'gray' ? 'group-hover:text-gray-600 dark:group-hover:text-gray-300' : tool.color === 'orange' ? 'group-hover:text-orange-600 dark:group-hover:text-orange-400' : 'group-hover:text-pdfred dark:group-hover:text-red-400'} transition-colors`}>{tool.name}</h3>
